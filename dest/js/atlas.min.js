...
function filterData(filters) {
    ...
    if (filters) {
        for (var pub_key in _data['publication']) {
            var row = _data['publication'][pub_key];
            // Check filters
            ...
            for (var i in filters) {
                ...
                if (i == 3) {
                    collab_key = match;
                    match = _data['entity'][collab_key]['entity_name'];
                }
            ...
            if (addToDisplay) {
                var url = _data['publication'][pub_key]['downloadURL'];
                filteredRecords[pub_key] = [row, url];
            }
        }
    }
...
function createTableBody(list) {
    ...
    var msg = " publications match these search results. See ";
    msg += '<a target="_top" href="http://www.sciencepolicyatlas.com/" style = "color: #ffffff">' + "tips" + "</a>";
    msg += " for leveraging the Atlas.";
    if(count === 1) {
        msg = " publication matches these search results. See" + '<a target="_top" href="http://www.sciencepolicyatlas.com/" style = "color: #ffffff">' + "tips" + "</a>" + "for leveraging the Atlas.";
    }
...
function createTableRow(id, org) {
    ...
    for (var i in tableCols) {
        ...
        if (column == 'name') {
            string += "<a target=\"_blank\" href=\"" + org[1] + "\">" + org[0][column].substring(0, org[0][column].length-4) + "</a>";
        } else if (column == 'entity_uploader') {
            var name = _data['entity'][org[0][column]]["entity_name"];
            string += "<a target=\"_blank\" href=\"http://www.sciencepolicyatlas.com/collaboration?collab=" + org[0][column] + "\">" + name + "</a>";
        } else {
            string += org[0][column];
        }
        string += "</td>\n";
...
function createFilterFor(key, index) {
    ...
    var options = [];
    for (var pub in _data['publication']) {

        if (key == "entity_uploader") {
            var collab_key = _data['publication'][pub][key];
            var values = [_data['entity'][collab_key]['entity_name']];
        } else {
            var values = [_data['publication'][pub][key]];
        }
