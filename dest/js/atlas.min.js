...
function filterData(filters) {
    ...
    if (filters) {
        for (var org_key in _data) {
            for (var rec_key in _data[org_key]) {
                var row = _data[org_key][rec_key];
                // Check filters
                ...
                if (addToDisplay) {
                    filteredRecords[rec_key] = row;
                    filteredRecords[rec_key]["link"] = org_key;
                    filteredRecords[rec_key]["URL"] = _data[org_key][rec_key]["downloadURL"];
                }
            }
        }
    }
...
function createTableRow(id, org) {
    ...
    for (var i in tableCols) {
        ...
        if (column == 'name') {
            string += "<a target=\"_top\" href=\"" + org["URL"] + "\">" + org[column].substring(0, org[column].length-4) + "</a>";
        }
        else if (column == 'link'){
            var ref = new Firebase('https://atlas-new-format.firebaseio.com/collaborations');
            ref.orderByKey().startAt(org[column]).endAt(org[column]).once('value', function(snapshot) {
                var c_data = snapshot.val();
                // var c_name = c_data.child("organization_name").val();
            });
            string += "<a target=\"_top\" href=\"http://www.sciencepolicyatlas.com/collaboration?collab=" + org[column] + "\">" + "c_name" + "</a>";
        }
        else {
            string += org[column];
        }
